            ...
            updateStatus(i,event,id)
            {
                ...
                AssignSaleUser()
                {
                    ...
                    .subscribe(d => {
                        ...
                        console.log('sales executive error')
                    });
                }

                selectSales(type:any)
                {
                    if(type=='from'){
                    this.transferData.from_sales_mobile = this.sales_users.filter( x => x.id === this.transferData.from_id )[0].phone;
                    }
                    if(type=='to')
                    {
                        this.transferData.to_sales_mobile = this.sales_users.filter( x => x.id === this.transferData.to_id )[0].phone; 
                    }
                }

                onToggleClose(){
                    this.transfer_karigerbtn=false;
                    this.transferData = {}; 
                }

                transferData:any={};
                transfer_karigerbtn=false;
                transferKaigers(f:NgForm){
                    this.dialog.confirmation(" Transfer Data - (Transfered Data Is Not Reversible)")
                    .then(resp=>{
                        console.log(resp);
                        if(resp){
                        console.log('run successfully');
                        // f.resetForm();
                    this.db.post_rqst({'data' : this.transferData}, 'master/TRANSFER_USER_DATA')
                    .subscribe(d => {
                        console.log(d);
                        f.resetForm();
                        this.toggle = "false";
                        this.transferData = {};
                        this.dialog.success( 'Karigers Transfered successfully ');
                        this.getSalesList('');
                    });
                        }
                })
